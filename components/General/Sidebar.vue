  ...
  <div class="columns is-multiline mt-2">
    ...
    >
      <div
        ...
        slot="trigger"
        slot-scope="props"
        :class="props.open ? 'catopen' : 'catclosed'"
      >
        <img
          v-if="props.open"
          :src="require('@/assets/sidebar/minus_active.png')"
        />
        <img
          v-if="!props.open"
          :src="require('@/assets/sidebar/plus_passive.png')"
        />
        {{ category.title }}
      </div>
      <b-collapse
      ...
      >
        <div class="p-1" slot="trigger" slot-scope="props">
          <img
            v-if="subcategory.items && props.open"
            :src="require('@/assets/sidebar/minus_active.png')"
          />
          <img
            v-if="subcategory.items && !props.open"
            :src="require('@/assets/sidebar/plus_active.png')"
          />
          <img
            v-if="!subcategory.items"
            :src="require('@/assets/sidebar/sub_arrow.png')"
          />
          {{ subcategory.title }}
        </div>
        <div
          class="column is-12 item p-1 pl-5"
          v-for="(item, i3) of subcategory.items"
          :key="i3"
        >
          {{ item }}
        </div>
