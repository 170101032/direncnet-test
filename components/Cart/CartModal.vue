<template>
  <div class="columns is-multiline cartmodal p-4">
    <a @click="$parent.close()" class="closex"
      ><img src="@/assets/signin/popupClose.png"
    /></a>
    <div class="column is-12 cartmodal-title">Urun Sepetinize Eklendi</div>
          ...
          <td class="column is-2 table-box p-2">
            <div class="column is-12 table-box p-2 pb-4">
              ...
              <a
                ...
                class="table-box p-2 px-3 mx-1 downquan"
                >-</a
              >
              {{ item.quantity }}
              <a @click="item.quantity++" class="table-box p-2 px-3 mx-1 upquan"
                >+</a
              >
            </div>
          </td>
          <td class="column is-2 table-box p-2">
            {{ item.price.toPrecision(5) }} TL + %18 KDV
          </td>
          <td class="column is-2 table-box p-2">
            {{ (item.price * item.quantity).toPrecision(5) }} TL
          </td>
          <td class="column is-1 table-box p-2">
            <a @click="removeCart(item)" class="cartmodal-cross p-1"
              ><img src="@/assets/cart/cross.png"
            /></a>
          </td>
        </tr>
      </table>
    ...
    <div class="column is-12" align="right">
      <div class="column is-4 table-box p-1" align="right">
        ...
        {{
          cart.reduce((acc, e) => acc + e.price * e.quantity, 0).toPrecision(5)
        }}
        TL
      </div>
    </div>
    ...
    <div class="column is-12 border-top mt-3 p-2 pt-3">
      <a class="cartmodal-button p-2" @click="$parent.close()"
        >Alisverise Devam</a
      >
      <NuxtLink class="cartmodal-button p-2 is-pulled-right" to="/CartPage"
        >Sepete Git</NuxtLink
      >
    </div>
  </div>
